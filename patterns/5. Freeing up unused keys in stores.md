## Freeing up unused keys in stores

Store module key-value storage can hold at most 1GB. It is usually enough if used correctly, but it is still a good idea (and sometimes even necessary) to free up unused keys.
...
Here's the trick. Since we don't need these counters outside of the hourly window, we can safely delete these counters for the previous hourly window and free up the memory.
